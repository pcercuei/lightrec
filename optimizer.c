...
static bool lightrec_detect_busywait_range(struct block *block, u16 min, u16 max)
{
	struct opcode *list = block->opcode_list;
	u64 read_mask = 0;
	u64 write_mask = 0;
	...
	pr_debug("Found busy-wait loop at PC 0x%08x (0x%x -> 0x%x)\n",
		 block->pc, min << 2, max << 2);
	return true;
}

int lightrec_detect_busywait(struct block *block)
{
	struct opcode *op;
	unsigned int i;
	s32 offset;
	...
	for (i = 0; i < block->nb_ops; i++) {
		...
		if (op_flag_local_branch(op->flags) && has_delay_slot(op->c)) {
			...
			if (lightrec_detect_busywait_range(block, offset, idx))
