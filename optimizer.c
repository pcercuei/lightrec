...
static void lightrec_transform_to_nops(struct opcode_list *list)
{
	/* Transform all opcodes detected as useless to real NOPs
	 * (0x0: SLL r0, r0, #0) */
	for (; list; list = SLIST_NEXT(list, next))
		if (is_nop(list->opcode))
			list->opcode.opcode = 0x0;
}

static void (*lightrec_optimizers[])(struct opcode_list *) = {
